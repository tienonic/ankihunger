  ...
  return (
    <div>
      ...
      <Show when={tipsVisible()}>
        <div class="key-hints">
          <kbd>Space</kbd>/<kbd>F</kbd> flip &middot; <kbd>1</kbd>-<kbd>4</kbd> rate &middot; <kbd>D</kbd> flip/good &middot; <kbd>/</kbd> note
        </div>
      </Show>

      <div class="flashcard-nav">
        <span class="flash-state-badge">{STATE_NAMES[s.cardState()] ?? 'New'}</span>
        <span class="flash-due-count">{s.dueCount().due} due / {s.dueCount().total} total</span>
      </div>
    </div>
  );
}
